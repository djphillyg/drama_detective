...
class DriftDetectorAgent:
    ...
    def check_drift(self, question: str, answer: str, session_id: Optional[str] = None) -> dict:
        # Build user prompt
        user_prompt = build_drift_detector_prompt(question, answer)

        # Call Claude API
        response = self.client.call(DRIFT_DETECTOR_SYSTEM, user_prompt, session_id=session_id)
